...
export default function BanglaNewsEditor() {
  ...
  const handleReplace = (wrong, correct) => {
    ...
    setTooltip(null); setChecked(false); setErrors([]);
  };
    ...
    <div onClick={() => setTooltip(null)} style={{
      ...
      <header style={{
        ...
        <h1 style={{ fontSize: "clamp(22px, 5vw, 36px)", color: C.text, margin: 0, fontWeight: 900, lineHeight: 1.2 }}>
          বাংলা <span style={{ color: C.accent }}>নিউজ</span> এডিটর
        </h1>
        <div style={{
          display: "flex", justifyContent: "center", gap: "14px", marginTop: "10px",
          fontSize: "10px", color: C.textMuted, fontFamily: "'JetBrains Mono', monospace", flexWrap: "wrap"
        }}>
          {[["প্রথম আলো", C.accent], ["bdnews24.com", C.accent2], ["বাংলা একাডেমি", C.green]].map(([t, c]) => (
      ...
      <div style={{ width: "100%", maxWidth: "880px", padding: "20px 16px 60px" }}>
            ...
            <div>
              <div style={{ position: "relative" }}>
                ...
                <textarea
                  ...
                  onChange={e => { setText(e.target.value); setChecked(false); setErrors([]); setTooltip(null); }}
        ...
        <div style={{
          ...
          <div style={{ padding: "10px 16px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "5px" }}>
            {[
              ["ণ-ত্ব / ষ-ত্ব", "ঋ/র/ষ → ণ"],
              ["ই/ঈ-কার", "তদ্ভবে ই, তৎসমে ঈ"],
              ["কী / কি", "প্রশ্নে কী, অব্যয়ে কি"],
              ["সংখ্যা", "বাংলা অঙ্ক ব্যবহার"],
              ["দেশের নাম", "যুক্তরাষ্ট্র/যুক্তরাজ্য"],
              ["উদ্ধৃতি", "একক ' ' চিহ্ন"],
            ].map(([t, d]) => (
              <div key={t} style={{ padding: "6px 8px", background: C.surface, borderRadius: "5px" }}>
                <div style={{ fontWeight: 700, fontSize: "10px", color: C.accent }}>{t}</div>
                <div style={{ fontSize: "9px", color: C.textMuted }}>{d}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{
          marginTop: "14px", fontSize: "10px", color: C.textLight,
          fontFamily: "'JetBrains Mono', monospace", textAlign: "center"
        }}>Powered by Gemini 2.5 Flash (Free) · প্রথম আলো ও bdnews24.com সম্পাদকীয় রীতি</div>
      </div>
