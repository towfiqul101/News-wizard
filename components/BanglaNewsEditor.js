...
export default function BanglaNewsEditor() {
  ...
  // Replace one specific error and remove it from list
  ...
  // Build highlighted segments — stays alive during manual editing
  const buildSegments = () => {
  ...
  return (
    <div onClick={() => setTooltip(null)} style={{
      ...
      <header style={{
        ...
        <h1 style={{ fontSize: "clamp(22px, 5vw, 36px)", color: C.text, margin: 0, fontWeight: 900 }}>
      ...
      <div style={{ width: "100%", maxWidth: "880px", padding: "20px 16px 60px" }}>
        ...
        {apiError && (
          <div style={{
            ...
            <button onClick={() => setApiError(null)} style={{ background: "none", border: "none", color: C.textMuted, cursor: "pointer", fontSize: "16px" }}>✕</button>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <ActionBtn onClick={rewriteNews} disabled={loading || !text.trim()}
              ...
              loadingText="সম্পাদনা হচ্ছে..." text="📰 সম্পাদনা + ইংরেজি" />
          </div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <textarea
                  ...
                  onChange={e => {
                    ...
                    setText(newText);
                    setTooltip(null);
                    // Keep errors alive — only drop ones that no longer exist in text
            ...
            <div style={{ maxHeight: "500px", overflowY: "auto" }}>
              ...
              {checked && errors.length === 0 && (
                <div style={{ padding: "50px 22px", textAlign: "center", animation: "fadeIn 0.3s ease" }}>
                  ...
                  <div style={{ color: C.textMuted, fontSize: "12px", marginTop: "6px" }}>বাংলা একাডেমি বানানরীতি (২০১২) অনুযায়ী সঠিক।</div>
                </div>
              )}
              {checked && errors.map((err, i) => (
                <div key={i} style={{
              ...
              {rewriteResult && (
                <div style={{ animation: "fadeIn 0.3s ease" }}>
                  ...
                  <div style={{ padding: "18px 20px", borderBottom: `1px solid ${C.border}` }}>
                    <div style={{ fontSize: "10px", letterSpacing: "3px", color: C.accent, textTransform: "uppercase", fontWeight: 700, marginBottom: "10px", fontFamily: "'JetBrains Mono', monospace" }}>বাংলা সম্পাদিত সংস্করণ</div>
                    ...
                    {rewriteResult.headline && (
                      <h2 style={{ fontSize: "21px", color: C.text, fontWeight: 800, margin: "0 0 12px", lineHeight: 1.5, borderLeft: `3px solid ${C.accent}`, paddingLeft: "14px" }}>
                        {rewriteResult.headline}
                      </h2>
                    )}
                  ...
                  <div style={{ padding: "18px 20px", borderBottom: `1px solid ${C.border}`, background: "#f8fafc" }}>
                    <div style={{ fontSize: "10px", letterSpacing: "3px", color: C.accent2, textTransform: "uppercase", fontWeight: 700, marginBottom: "10px", fontFamily: "'JetBrains Mono', monospace" }}>ENGLISH VERSION</div>
                    ...
                    {rewriteResult.english_headline && (
                      <h2 style={{ fontSize: "19px", color: "#1a2a3a", fontWeight: 700, margin: "0 0 10px", lineHeight: 1.4, fontFamily: "'Cormorant Garamond', Georgia, serif", borderLeft: `3px solid ${C.accent2}`, paddingLeft: "14px" }}>
                        {rewriteResult.english_headline}
                      </h2>
                    )}
                    <div style={{ fontSize: "14px", color: "#4a5568", lineHeight: 1.8, whiteSpace: "pre-wrap", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                      {rewriteResult.english_version}
                    </div>
                  </div>
                  ...
                  {rewriteResult.editorial_notes && (
                    <div style={{ padding: "14px 20px", background: "#f0fdf4" }}>
                      <div style={{ fontSize: "10px", letterSpacing: "3px", color: C.green, textTransform: "uppercase", fontWeight: 700, marginBottom: "8px", fontFamily: "'JetBrains Mono', monospace" }}>সম্পাদকীয় মন্তব্য</div>
                      <div style={{ fontSize: "12px", color: "#4a7c59", lineHeight: 1.7 }}>{rewriteResult.editorial_notes}</div>
                    </div>
                  )}
                </div>
              )}
        ...
        <div style={{ marginTop: "18px", textAlign: "center" }}>
          <div style={{ fontSize: "10px", color: C.textMuted, fontFamily: "'JetBrains Mono', monospace", marginBottom: "4px" }}>
            বাংলা একাডেমি প্রমিত বানানের নিয়ম (২০১২) · Powered by Gemini AI
          </div>
    ...
    <button onClick={onClick} disabled={disabled} style={{
      ...
      {isLoading ? (
        <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <span style={{ display: "inline-block", width: "12px", height: "12px", border: "2px solid #ddd", borderTopColor: color, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
          {loadingText}
        </span>
      ) : text}
    </button>
...
function StatusPill({ bg, color, text }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", background: bg, color, padding: "3px 12px", borderRadius: "20px", fontSize: "11px", fontWeight: 700 }}>
      {text}
    </div>
  );
}
...
function SmallBtn({ onClick, text, bg = "#f5f0e8", color = "#8a7e6e" }) {
  return (
    <button onClick={onClick} style={{ padding: "4px 12px", background: bg, color, border: `1px solid ${bg === "#f5f0e8" ? "#e8e0d4" : bg}`, borderRadius: "6px", fontSize: "11px", cursor: "pointer", fontWeight: 600 }}>
      {text}
    </button>
  );
}
...
function EmptyState({ text }) {
  return <div style={{ padding: "50px 22px", textAlign: "center", color: "#b5a898", fontSize: "13px" }}>{text}</div>;
}
